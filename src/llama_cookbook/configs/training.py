...
class train_config:
    ...
    action_chunk_size: int = 1 # number of actions decoded per token embedding
    ...
    multi_label_bce: bool = False # if True, use multi-label binary cross entropy loss for trajectory prediction
    action_chunk_size: int = 1 # number of actions decoded per token embedding
    action_model_type: str = "default" # "default" or "diffusion", only applicable when action_head is True. "diffusion" will use a diffusion model for action prediction, which is more computational expensive but can achieve better performance.
    action_head_use_mon: bool = False # if True, use MoN (Minimum over N) loss for the action head, injecting Gaussian noise into action embeddings
